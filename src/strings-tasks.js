...
function removeFirstOccurrences(str, value) {
  ...
  return ind === -1 ? str : str.slice(0, ind) + str.slice(ind + value.length);
}
...
function isPalindrome(str) {
  const reg = str.toLowerCase().replace(/[\W_]/g, '');
  return reg === reg.split('').reverse().join('');
}
...
function findLongestWord(sentence) {
  const lengths = sentence.split(' ');
  let maxLength = lengths[0];
  lengths.forEach((item) => {
    const itemLength = item.length;
    maxLength = maxLength.length < itemLength ? item : maxLength;
  });
  return maxLength;
}
...
function invertCase(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    invertStr += ch === ch.toUpperCase() ? ch.toLowerCase() : ch.toUpperCase();
  }
  return invertStr;
}
...
function encodeToRot13(str) {
  let codeString = '';
  codeString = str.split('').map((item) => {
    if (item.charCodeAt(0) < 78) {
      return item.charCodeAt(0) < 65
        ? item.charCodeAt(0)
        : item.charCodeAt(0) + 13;
    }
    if (item.charCodeAt(0) < 110) {
      return item.charCodeAt(0) <= 90
        ? item.charCodeAt(0) - 13
        : item.charCodeAt(0) + 13;
    }
